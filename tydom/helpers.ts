...
>(
  ...
  if (cacheMap.has(uri)) {
    ...
    if (now < time + DEBOUNCE_TIME) return promise as Promise<T>;
  }
  const promise = client.get<TydomEndpointDataResponse>(uri).then((res) => {
    if (res.error > 0) {
      ...
      if (res.error > 10) {
        debug(
          `Accessory with uri=${uri} seems unreacheable (error=${res.error}).`,
        );
        throw new Error('UnreacheableAccessory');
      } else {
        debug(
          `Accessory with uri=${uri} is reporting unknown issues (error=${res.error}).`,
        );
      }
    }
...
export const getEndpointDetailsFromMeta = (
  ...
  assert(
    details,
    `Endpoint with id="${endpointId}" not found in device with id="${deviceId}" meta`,
  );
...
export const getEndpointSignatureFromMetadata = (
